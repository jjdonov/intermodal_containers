defmodule IntermodalContainers.ContainerCode.Parser do
  ...
  defp parse_step({_rest, 0, %{}} = parse_state) do
    consume(parse_state, 1, &accept_l/1 ) # fn s -> %{ s |  }) #idea: if we pass updateFn
  end
  ...
  defp accept_l(size_code) do
    ...
    cond do
      byte_size(size_code) != 1 ->
        {:error, "length code should be 1 character long. #{size_code} is #{byte_size(size_code)}"}
